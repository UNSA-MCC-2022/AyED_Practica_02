...
BTree.prototype.addControls = function () {
	...
	for (i = MIN_MAX_DEGREE; i <= MAX_MAX_DEGREE; i++) {
		radioButtonNames.push(String(i));
	}
	...
	if (tree != null) {
		...
		if (i == tree.numKeys) {
			if (!tree.isLeaf) {
				...
				this.cmd("SetHighlight", tree.graphicID, 0);
				this.cmd("SetText", this.messageID, "Elemento " + val + " no encontrado");
			}
		}
			...
			this.cmd("SetText", this.messageID, "Elemento " + val + " encontrado");
			this.cmd("Step");
...
BTree.prototype.insertNotFull = function (tree, insertValue) {
	...
	this.cmd("Step");
	if (tree.isLeaf) {
		this.cmd("SetText", this.messageID, "Insertando " + insertValue + ".  Insertando en hijo");
		tree.numKeys++;
...
BTree.prototype.split = function (tree) {
	this.cmd("SetText", this.messageID, "El nodo tiene muchos elementos, separar ...");
	this.cmd("SetHighlight", tree.graphicID, 1);
	this.cmd("Step");
	this.cmd("SetHighlight", tree.graphicID, 0);
	...
	if (tree.parent != null) {
		...
		if (parentIndex == currentParent.numKeys + 1) {
			throw new Error("No se puede encontrar el hijo");
		}
	...
	if (tree != null) {
		...
		if (i == tree.numKeys) {
			...
			if (tree.isLeaf) {
				...
				this.cmd("SetText", this.messageID, "Verificando si el elemento izquierdo se puede eliminar");
				this.cmd("SetEdgeHighlight", tree.graphicID, tree.children[i].graphicID, 1);
				...
				if (tree.children[i].numKeys == this.min_keys) {
					...
					this.cmd("SetText", this.messageID,
						"Verificando si el elemento izquierdo se puede eliminar");
					this.cmd("SetEdgeHighlight", tree.graphicID, tree.children[i + 1].graphicID, 1);
					this.cmd("Step");
					this.cmd("SetEdgeHighlight", tree.graphicID, tree.children[i + 1].graphicID, 0);
					...
					if (tree.children[i + 1].numKeys == this.min_keys) {
						this.cmd("SetText", this.messageID,
							"El sub-arbol no tiene nodos extra");
						this.cmd("Step");
						this.cmd("SetTextColor", tree.graphicID, FOREGROUND_COLOR, i);
						nextNode = this.mergeRight(tree.children[i]);
						this.doDeleteNotEmpty(nextNode, val);
						...
						this.cmd("SetText", this.messageID,
							"Verificando si el elemento derecho se puede eliminar");
						this.cmd("Step");
					...
					this.cmd("SetText", this.messageID,
						"TVerificando si el elemento izquierdo se puede eliminar");
					this.cmd("Step");
